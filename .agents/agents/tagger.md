...
> **You enforce structural tags** (for example `empire`, `biography`) inside `tags` so structure is inferred from semantics, not filenames.
...
### Step 4 - Build Robust Canonical Tags
...
If for any reason the tag array could not be brought to PASS after retries, write `validation: FAIL`
so formatter knows to re-run the validator independently.
