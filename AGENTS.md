...
<vault root>/
  ...
  .agents/
    ...
    taxonomy.json               ← the ONLY tag registry; tools and docs read it
...
and domain/category are the first two tags. `note:` belongs to the user; no agent
writes to it.

### The Tag Array

Tags are **positional**. Order is schema, not style.

| # | Slot | Count | Where it comes from |
|---|---|---|---|
| 1 | domain | 1 | the top-level folder |
| 2 | category | 1 | `taxonomy.folderMap`, longest prefix wins |
| 3 | type | 1 | the note's form: `person`, `empire`, `event`, `concept`, `work`, `ruling`, `creed`, `technique`, `compendium`, `overview` |
| 4 | themes | 1–3 | closed registry — **the linking substrate** |
| 5 | entities | 0–6 | open vocabulary, kebab-case |
| 6 | marker | 1 | `cli` (pipeline-written) or `manual` (hand-written) |

Slots 1, 2, and 6 are mechanical. Slot 3 is near-mechanical. Only themes and entities
need judgement, and themes come from a fixed list.

**Themes are the point.** Entity tags are nearly unique per note, so they cannot
connect anything. Themes are what two different notes genuinely share, and they are
what `get_related_notes.ps1` matches on. A note without a theme tag can never be
linked to anything — validation rejects it.

Every registry lives in `.agents\taxonomy.json` and nowhere else. The four registries
are mutually disjoint, so a tag's slot is unambiguous from the tag alone. To change
the taxonomy, edit that file; `validate_tags.ps1` reads it directly and
`audit_skill_sync.ps1` verifies the docs still agree.

Full rules: `.agents\skills\obsidian_yaml_enforcer\SKILL.md`.

### Table of Contents

**Every `cli` note carries a Table of Contents.** There is no word-count threshold.
The sole exception is a note with fewer than two content headings, where a ToC would
have nothing to list — and that itself signals a generation run that produced too few
sections. `manual` notes are exempt; they belong to the user.

Generated by `.agents\tools\generate_toc.ps1`, which is safe to re-run.

### Map of Contents (MOC)

**Only five domains have a MOC**: Art, History, Islam, Literature, Science.

- **`Reason/` has none.** Its index is the owner's hand-written `Chain Of Thoughts.md`,
  which is sacred — never touch, replace, or index over it.
- **Nested / sub-category MOCs are retired.** One MOC per domain. Adding sub-indexes
  is the vault owner's decision, not an agent's.

Each lives at `<Domain>\Map of Contents - <Domain>.md`, is grouped by category tag,
and is regenerated rather than hand-edited:
...
MOC files are tagged exactly `[<domain>, moc, cli]` and are exempt from the
positional schema.
...
STAGE 4: ASSEMBLY (weaver)
  ...
  THEN assemble:
    ...
    - Insert `- - -` separators
    - (ToC is generated by tool in Stage 5b, after the file is on disk)
...
STAGE 5b: TABLE OF CONTENTS (weaver) — MANDATORY for cli notes

    powershell -File ".agents\tools\generate_toc.ps1" -FilePath "[full note path]"

  No word-count threshold. TOC_SKIPPED means the note has fewer than two
  content headings — report it as a thin-generation signal, do not hand-write one.
...
- [ ] `generate_toc.ps1` called → `TOC_WRITTEN` (or `TOC_SKIPPED` with reason).
...
- [ ] `update_moc.ps1` → `MOC_UPDATED`, `MOC_CREATED`, or `ALREADY_LISTED`
      (skipped entirely for Reason notes — that domain has no MOC).
